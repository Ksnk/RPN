...
class revpolnot_classTest extends PHPUnit_Framework_TestCase
{
    ...
    function testBuildingSyntaxTree()
    {
        ...
        foreach ([
            ...
            $this->assertEquals('[]', json_encode($r->errors));
            $this->assertEquals($k . "\n" . $v, $k . "\n" . json_encode($r->ev($k)));
        }
    }
    ...
    /**
     * “естова€ функци€ дл€ проверки логического модул€
     * –еализаци€ исчислени€ тега - сравнение значени€ на > c константой
     * @param $op
     * @return bool
     */
    function _celOp($op)
    ...
    /**
     * проверка числового калькул€тора
     */
            ...
            $result=$r->ev($k,
                function ($op) use($r) {
                    ...
                    if(!is_array($op) || !isset($op['data']))
                        ...
                        $result=0+$op['data'] ; // €вное преобразование к числу
                    }
                    return $result;
                },
