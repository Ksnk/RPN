...
Разместите весь комплект файлов в соответствии с PSR-0.

Ретрансляция шаблонов может быть сделана так

Разместите в файле index.php такой код
...
* TEMPLATE_PATH - каталог, в котором находятся исходники шаблонов
* PHP_PATH - каталог, в котором будут размещены оттранслированные шаблоны. Во втором случае - TEMPLATE_PATH и TEMPLATE_PATH будут одинаковы.
* TEMPLATE_EXTENSION - расширение шаблона

Нужно помнить, что при трансляции шаблона получаются "обычные" классы PHP. Если возникает желание использовать шаблон - вызывается метод `_` из этого класса. Сам класс, отличается от имени шаблона префиксом `tpl_`. Любой метод будет отличатся от имени мкро или блока префиксом `_`. Можно использовать такой класс-хелпер для вызова определеного шаблона.


    /**
     *  @param string|array $name - template name, or name-method combination.
     *
     */
    function template($name, $method, $par = array())
    {
        static $cache;
        if (is_array($method) && empty($par)) {
            $par=$method;
            $method='';
        }
        if (is_array($name)) {
...
* Variables - Так как не предполагается использовать объекты в качестве данных для рендеринга, то способы указания выборки элемента и выборки из массива полностью эквивалентны. Предполагается сделать "типизацию" при определении параметров макрокоманд, чтобы выборка из массива и получение метода объекта были оптимизированы. Также забавно было бы сделать "ретрансляцию" после нескольких прогонов. При прогоне шаблона собирается информация о типе переменных и при ретрансляции оптимизируется выборка в соответствии с полученными данными.
...
В результате работы транслятора получается некий php-класс. Имя класса зависит от имени шаблона. Из шаблона с именем `XXX.twig` - получается класс `tpl_XXX` При этом все точки в слове XXX заменяются на подчеркивания. При рендеринге данных достаточно проинициировать этот класс и вызвать либо нужную функцию, либо “корневую” - ‘_’. Никаких дополнительных действий и включения в код файлов компилятора не нужно. Дополнительные фильтры, функции и тесты, использующиеся при run-time рендеринге, описываются в классе tpl_base.
...
Описание тега loop вводится в конструкторе класса php_compiler
